...
class AuthHandler:
    def __init__(self, auth_secret):
        ...
        if ok:
            ...
            log("Failed to verify authenticator exiting", mycolors.FAIL)
            sys.exit(0)
    ...
    def verify_request(
        self, req_session: requests.Session, user_Id, metadata_challengeId
    ):
        """
    ...
    def validate_2fa(self, response, request_session):
        ...
        verification_token = self.verify_request(
            request_session, senderid, metadata_challengeid
        )
        # send the continue request
        self.continue_request(
            request_session, challengeid, verification_token, metadata_challengeid
        )
